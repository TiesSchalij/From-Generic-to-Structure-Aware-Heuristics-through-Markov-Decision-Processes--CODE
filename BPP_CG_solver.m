...
if saveKPs
    ...
    saveFolderFailedKP  = 'FailedKPfromBPP_200items_200lb700ub1000C';
    savedFailedKPinstances = cell(1);
end
...
while newColumnFound
    ...
    if round(1 - sum(v.*newColumn),12) < 0 %newColumn has negative reduced cost
        ...
        if saveKPs
            savedFailedKPinstances{nColsGeneratedBP+1} = s;
        end
        BPstart = tic;
...
if saveKPs
    ...
    savedKPinstances = savedFailedKPinstances; %it has to be saved under the correct name
    arrayNumber = numel(dir(saveFolderFailedKP)) -1; %should be the same, for robustness
    save(strcat(saveFolderFailedKP,'\','instanceArray',string(arrayNumber) ),'savedKPinstances')
